...
// HOW IT WORKS
// ─────────────────────────────────────────────────────────────────────────────
// Violentmonkey (and Tampermonkey) isolate the userscript from the real page JS
// regardless of @inject-into / @grant settings. The only 100% reliable way to
// run code in the real page world is to append a <script> element to the DOM.
//
// That injected <script> runs pageMain() which:
//   1. Wraps EventTarget.prototype.addEventListener so that when OSM's id.js
//      registers its DOMContentLoaded listener, we intercept and wrap it.
//   2. Inside the wrapped handler, window.iD is guaranteed to exist (OSM is
//      about to call iD.coreContext() itself), so we hook coreContext.
//   3. After context.init() runs, we push extra rendererBackgroundSource entries
//      into iD's imagery array — identical to the Strava Heatmap extension's
//      technique — then call ui().restart() to rebuild the Background panel.
// ─────────────────────────────────────────────────────────────────────────────

(function injectPageScript() {
    'use strict';

    // pageMain() runs in the real page context — no closures over the outer scope.
    function pageMain() {
        ...
        async function applySlots(context) {
            ...
            // Restart the UI to rebuild the Background list with our new entries.
            // We must restart even when the editor has unsaved data; the guard was
            // causing the extra slots to never appear.
        ...
        window.__iDExtraBg = {
            version: '0.2.0',
            hooked: false,
            note: 'run localStorage.setItem("iD-extra-bg-debug","1") then reload for verbose logs',
        };

        // Intercept EventTarget.prototype.addEventListener so we can wrap every
        // DOMContentLoaded bubble listener that gets registered after us.
        // This catches OSM's id.js handler regardless of when that file executes.
        // At the moment the handler body runs, window.iD is guaranteed to be
        // defined (the handler itself would fail without it).
        const _origAEL = EventTarget.prototype.addEventListener;
        EventTarget.prototype.addEventListener = function iDExtraBgInterceptor(type, fn, opts) {
            if (this === document && type === 'DOMContentLoaded') {
                const isCapture = opts === true || (opts && opts.capture);
                if (!isCapture && typeof fn === 'function') {
                    const orig = fn;
                    fn = function (event) {
                        dbg('intercepted DOMContentLoaded handler, window.iD=', typeof window.iD);
        ...
        // Restore original once DOMContentLoaded fires — no need to keep intercepting.
        _origAEL.call(document, 'DOMContentLoaded', function restore() {
            EventTarget.prototype.addEventListener = _origAEL;
            dbg('addEventListener restored');
        });

        dbg('bootstrap complete, waiting for OSM handler registration');
    }

    // Inject pageMain() into the real page JS world.
